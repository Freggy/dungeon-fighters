...
public class AStar {
    ...
    /**
     * Find the shortest path between a given start and end node using the A* algorithm if a path exists.
     *
     * @param nodes all nodes to inspect
     * @param start starting node
     * @param end   end node
     * @return a list of nodes representing the shortest path from <code>start</code> to <code>end</code>. <code>null</code> if there is none
     */
     ...
     * @return a list of nodes representing the shortest path from <code>start</code> to <code>end</code>. <code>null</code> if there is none
     */
    public static <T> List<Node<T>> findPath(Node<T>[] nodes, Node<T> start, Node<T> end, Collection<Node<T>> excluded) {
        // We have to reset all parents so that older cycles through the same array won't affect this one.
        ...
        while (!openNodes.isEmpty()) {
            ...
            for (Node<T> successor : successors) {
                if (successor == null || closedNodes.contains(successor) || excluded.contains(successor)) continue;
